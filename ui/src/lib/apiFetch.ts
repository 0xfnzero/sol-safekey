...
const TAURI_SECURE_PROXY_HEADER = "x-sol-safekey-tauri-secure-proxy";
...
async function getSecureBodyPublicKey(): Promise<CryptoKey> {
  ...
  if (!secureKeyPromise) {
    secureKeyPromise = (async () => {
      const session = await fetchSecureSession();
...
export async function apiFetch(
  ...
  try {
    ...
    const response = await sendApiRequest(cleanPath, nextInit);
    ...
    if (SECURE_KEY_DECRYPT_ERRORS.has(error ?? "")) {
      ...
      return sendApiRequest(cleanPath, retryInit);
    }
