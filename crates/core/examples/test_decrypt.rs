...
//!
//! Usage: cargo run -p fnzero-safe-core --example test_decrypt <password>

use fnzero_safe::{decrypt_key_to_bytes, generate_encryption_key_simple, KeyManager, Signer};
...
fn main() {
    ...
    let password = if args.len() >= 2 {
        ...
        eprintln!("Usage: cargo run -p fnzero-safe-core --example test_decrypt <password>");
        std::process::exit(1);
    };
