...
enum Commands {
    ...
    SolOps {
        ...
        #[command(subcommand)]
        command: fnzero_safe::solana_utils::SolanaOpsCommand,
    },
}
...
fn print_colored_help() {
    ...
    println!(
        "     {} {}",
        "$".bright_white(),
        "fnzero-safe start".bright_green()
    );
    println!();
    ...
    println!(
        "     {} {}",
        "$".bright_white(),
        "fnzero-safe setup-2fa".bright_green()
    );
    println!(
        "     {} {}",
        "$".bright_white(),
        "fnzero-safe gen-2fa-wallet -o wallet.json".bright_green()
    );
    println!(
        "     {} {}",
        "$".bright_white(),
        "fnzero-safe unlock-2fa-wallet -f wallet.json".bright_green()
    );
    println!();
    ...
    println!(
        "     {} {}",
        "$".bright_white(),
        "fnzero-safe sol-ops -f wallet.json balance".bright_green()
    );
    println!(
        "     {} {}",
        "$".bright_white(),
        "fnzero-safe sol-ops -f wallet.json transfer -t <地址> -a 0.1".bright_green()
    );
    println!();
    ...
    println!(
        "   - 运行 {} 查看某个命令的详细说明",
        "fnzero-safe <COMMAND> --help".bright_white()
    );
    println!();
}
...
fn check_password_strength(password: &str) -> Result<(), String> {
    // 检查密码长度下限
    if password.len() < fnzero_safe::MIN_PASSWORD_LENGTH {
        return Err(format!(
            "密码长度至少需要{}位",
            fnzero_safe::MIN_PASSWORD_LENGTH
        ));
    }

    // 检查密码长度上限
    if password.len() > fnzero_safe::MAX_PASSWORD_LENGTH {
        return Err(format!(
            "密码长度必须在{}-{}位之间",
            fnzero_safe::MIN_PASSWORD_LENGTH,
            fnzero_safe::MAX_PASSWORD_LENGTH
        ));
    }
...
fn main() {
    ...
    match cli.command.unwrap() {
                ...
                Ok(encrypted_data) => {
                    ...
                    match fs::write(&output, serde_json::to_string_pretty(&data).unwrap()) {
                        Ok(()) => {
                            ...
                            println!(
                                "   解锁命令: {}",
                                format!("fnzero-safe unlock-2fa-wallet -f {}", output)
                                    .bright_white()
                            );
                            println!();
                            println!(
                            ...
                            println!(
                                "   解锁命令: {}",
                                format!(
                                    "fnzero-safe unlock -f {}_keystore.json -p <主密码>",
                                    &public_key[..8]
                                )
                                .bright_white()
                            );
                            ...
                            println!(
                                "   {}",
                                format!(
                                    "fnzero-safe address -f {}_keystore.json -p <主密码>",
                                    &public_key[..8]
                                )
                                .bright_white()
                            );
        ...
        Commands::SolOps { file_path, command } => {
            ...
            let args = fnzero_safe::solana_utils::SolanaOpsArgs {
                command: command.clone(),
            };

            match fnzero_safe::solana_utils::execute_solana_ops(args, &file_path) {
                Ok(_) => {}
                Err(e) => {
