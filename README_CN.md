...
本指南提供了 **sol-safekey** 交互式菜单系统的完整 walkthrough。每个功能都有逐步说明和示例输出。
...
- **[2. 创建加密私钥(bot)（选项-2）](#2-创建加密私钥bot)（选项-2）** - 加密现有私钥并保存到 keystore
- **[3. 解密私钥（选项-3）](#3-解密私钥（选项-3）)** - 解密 keystore 并显示私钥
...
- **[17. Pump.fun 返现（查看与领取）](#17-pumpfun-返现（查看与领取））** - 查看并领取 pump.fun 返现（原生 SOL）
- **[18. PumpSwap 返现（查看与领取）](#18-pumpswap-返现（查看与领取））** - 查看并领取 PumpSwap 返现（WSOL）
